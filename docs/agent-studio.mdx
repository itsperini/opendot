...
When you click **Create identity**, OpenDot attaches the default four-stage
pipeline:
...
The platform API persists each identity and its default pipeline in PostgreSQL
as:
...
Editing an identity or its pipeline writes a new draft version. The current
...
Open [Configuration](/configuration) to tune the active identity's VAD, STT,
LLM, and TTS stages.
