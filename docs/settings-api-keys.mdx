...
The current prototype stores user-scoped API key hashes and metadata in PostgreSQL. The full token is only shown when a key is created, so copy it immediately if you need to use it outside the UI. Treat these as local development credentials, not production secrets.
