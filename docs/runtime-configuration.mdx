...
Dot devices, plus browser Speech-to-speech client-secret exchange.
...
Speech-to-speech Browser Test also uses `OPENAI_API_KEY`, but only inside the
runtime. The platform API mints a one-use realtime browser-session token at
`POST /api/runtime/realtime-browser-sessions`, the runtime exchanges it for an
OpenAI Realtime client secret at `/realtime/client-secret`, and the browser uses
that short-lived client secret for the WebRTC call.
