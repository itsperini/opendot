...
The current runtime endpoints are intended for local prototype use. They do not
yet enforce browser session or device authentication at the runtime boundary, so
avoid exposing the runtime directly on the public internet with real provider
keys.

The platform API stores the OSS OpenDot control plane in PostgreSQL: app users, local auth credentials, user preferences, API keys, versioned agents and pipelines, devices, device state, and deployments.
