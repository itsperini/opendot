...
The platform API stores the OpenDot control plane in PostgreSQL: app users, organizations, memberships, projects, environments, scoped API keys, versioned agents and pipelines, device state, deployments, sessions, events, and audit logs.
...
For local npm runs, leave `POSTGRES_URI` empty and the API/Drizzle commands use
...
When a Bearer token is provided, the API maps the Supabase JWT `sub` to `app_users.id`; product permissions still come from OpenDot membership tables, not Supabase `user_metadata`. For local Compose without Supabase, leave `OPENDOT_LOCAL_AUTH_DISABLED=false` and use the OpenDot auth page.
