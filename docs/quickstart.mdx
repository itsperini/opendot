...
Open `https://local.drizzle.studio` to inspect the PostgreSQL schema and rows, including `app_users`, `local_auth_credentials`, `user_preferences`, SDK API keys, versioned agents/pipelines, devices, device activation requests, device credentials, runtime session tokens, device state, and deployments.
...
## Start the voice runtime for pnpm runs
...
Open **Dot Device** to claim a spoken device code, refresh runtime availability, and bind the selected agent config to the device.
