---
title: Configuration
description: Tune the VAD, STT, LLM, and TTS stages for the active OpenDot agent identity.
---
...
configuration. The page separates the current modular pipeline from the future
...
The Speech-to-speech architecture mode is present as a placeholder for realtime
models that process audio input and produce audio output directly.
...
- **Speech-to-speech Architecture**: a placeholder for realtime model settings.
...
| Model                         | `gpt-5.1`                                      | Suggested OpenAI model or any custom compatible model id.       |
...
| Max output tokens             | blank                                          | Optional response token cap.                                    |
...
- **Suggested models**: current OpenAI model IDs such as `gpt-5.1`, `gpt-5`,
  and smaller GPT-5 / GPT-4.1 variants.
