...
void WifiConfigurationAp::StartWebServer()
{
    ...
    httpd_uri_t form_submit = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            if (buf_len > 1024) { // Limit request body size
    ...
    httpd_uri_t exit_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Delay callback so the HTTP response can flush
            ESP_LOGI(TAG, "Exiting config mode...");
            xTaskCreate([](void *ctx) {
                // Wait 200ms so the HTTP response can flush
                vTaskDelay(pdMS_TO_TICKS(200));

                auto* self = static_cast<WifiConfigurationAp*>(ctx);
                // Notify callback to exit config mode
    ...
    httpd_uri_t advanced_config = {
        ...
        .handler = [](httpd_req_t *req) -> esp_err_t {
            ...
            // Add configuration fields to JSON
                                                  ...
                                                  int32_t event_id, void *event_data)
{
    ...
    if (event_base == SC_EVENT){
        ...
        case SC_EVENT_GOT_SSID_PSWD:{
            ...
            // Connecting here would fail, so only save the credentials
            self->Save(ssid, password);
            // Delay exit from config mode
            xTaskCreate([](void *ctx){
...
void WifiConfigurationAp::Stop() {
    ...
    // Stop WiFi without deinit; WifiManager owns the WiFi driver
    esp_wifi_stop();
